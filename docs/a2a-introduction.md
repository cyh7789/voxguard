...
| Object | Purpose |
|--------|---------|
| **Agent Card** | Self-describing manifest (name, skills, URL, auth requirements) |
| **Message** | A communication turn — has a `role` (user/agent) and contains **Parts** |
| **Part** | Smallest content unit: `TextPart`, `FilePart`, or `DataPart` |
...
```
Task, Message, Part (TextPart, FilePart, DataPart),
...
|-----------|----------|----------|
| **TextPart** | `text` (string) | Natural language, prompts, responses |
| **FilePart** | `url` or `raw` bytes + `mediaType` | Images, PDFs, audio |
| **DataPart** | `data` (JSON object) + `mediaType` | Structured data, tool calls, forms |
...
| **FilePart** | §4.1.6 | ❌ We only use TextPart + DataPart, no file exchange | Low |
...
|-------------|-------------|-----------|-------------------|
| **Agent Card** | §4.4.1 | Use legacy `url` field (v0.3 style) | RC v1.0 uses `supportedInterfaces` array with `protocolBinding` and `protocolVersion` |
| **Capabilities** | §4.4.3 | Empty `AgentCapabilities()` | Should explicitly declare `streaming: false`, `pushNotifications: false` |
...
| **JSON-RPC method names** | §9.4 | `sync_client.py` uses `"message/send"` (v0.3 style) | RC v1.0 uses `"SendMessage"` (PascalCase) |
