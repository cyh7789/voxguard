"""
CAR-bench Codex agent under test.
...
class CARBenchAgentExecutor(AgentExecutor):
    """A2A executor that asks Codex for exactly one CAR-bench next action."""
    ...
    def __init__(
        ...
        *,
        ...
        self.client = CodexAppServerClient(
            ...
            cwd=os.getenv("CODEX_WORKDIR", "/tmp/car-bench-codex-workdir"),
            timeout_seconds=timeout_seconds,
            logger=logger.bind(role="agent_under_test", context="codex"),
        )
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            ...
            inference_result = self._call_codex_with_retries(
            ...
            if not has_tool_calls and context.context_id in self.ctx_id_to_turn_metrics:
                ...
                response_message.metadata.update(
                    {TURN_METRICS_KEY: metrics}
                )

            await event_queue.enqueue_event(response_message)

        except Exception as e:
            ctx_logger.error("Codex agent error", error=str(e), exc_info=True)
            response_message = new_message(
        ...
        *,
        ...
        for attempt in range(self.malformed_retries + 1):
            ...
            try:
                ...
                last_error = e
                correction = (
                    "The previous Codex output was invalid. Return one JSON object "
                    f"matching the schema. Error: {e}"
                )
                ctx_logger.warning(
        ...
        raise CodexMalformedResponseError(
            f"Codex did not produce a valid next-action JSON object: {last_error}"
        )
...
def parse_next_action(text: str) -> dict[str, Any]:
    try:
        ...
        if start == -1 or end == -1 or end <= start:
            raise CodexMalformedResponseError(f"No JSON object found in: {text[:200]}")
        payload = json.loads(text[start : end + 1])

    if payload.get("action") == "respond":
        content = payload.get("content")
        if not isinstance(content, str):
            raise CodexMalformedResponseError("respond action requires string content")
        return {"action": "respond", "content": content}

    if payload.get("action") == "tool_calls":
        tool_calls = payload.get("tool_calls")
        if not isinstance(tool_calls, list) or not tool_calls:
            raise CodexMalformedResponseError("tool_calls action requires non-empty tool_calls")
        normalized = []
        for tool_call in tool_calls:
            if not isinstance(tool_call, dict):
                raise CodexMalformedResponseError("each tool call must be an object")
            ...
            if not isinstance(tool_name, str) or not tool_name:
                raise CodexMalformedResponseError("each tool call requires tool_name")
            if not isinstance(arguments, dict):
                raise CodexMalformedResponseError("tool call arguments must be an object")
            normalized.append({"tool_name": tool_name, "arguments": arguments})
        return {"action": "tool_calls", "tool_calls": normalized}

    raise CodexMalformedResponseError("action must be either respond or tool_calls")


def _parse_tool_arguments_json(arguments_json: Any) -> dict[str, Any]:
    if not isinstance(arguments_json, str):
        raise CodexMalformedResponseError("tool call arguments_json must be a string")
    ...
    if not isinstance(parsed, dict):
        raise CodexMalformedResponseError("tool call arguments_json must decode to an object")
    return parsed
...
NEXT_ACTION_OUTPUT_SCHEMA = {
    ...
    "properties": {
        ...
        "content": {
            "type": "string",
            "description": "Natural user-facing assistant text when action is respond; otherwise empty.",
        },
        "tool_calls": {
            "type": "array",
            "description": "CAR-bench tool calls when action is tool_calls; otherwise empty.",
            "items": {
                "type": "object",
                ...
                "properties": {
                    ...
                    "arguments_json": {
                        "type": "string",
                        "description": "JSON object string containing the tool arguments, for example \"{}\" or \"{\\\"position\\\":50}\".",
                    },
                },
...
CODEX_DEVELOPER_INSTRUCTIONS = """You are an in-car assistant reasoning layer for CAR-bench.
